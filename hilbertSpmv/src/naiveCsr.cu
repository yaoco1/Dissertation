/**
 * @file naiveCsr.cu
 * @brief This file contains the Naive (scalar) CSR SpMV kernel and driver. It implements 
 * the simplest GPU SpMV method that is one CUDA thread per matrix row. Each thread iterates
 * sequentially over the non-zeros in its assigned row and accumulates the dot product independently.
 *
 * @author  Congcong Yao
 * @version 1.0
 * @date    07/08/2026
 ...
 *
 */
__global__ void naiveCsrSpmv(const int* rowPtr, const int* colIdx, const float* val, const float* x, float* y, int rows)
{
...
{
    if (argc < 2) {
        printf("Usage: ./naiveCsr <matrix.mtx>\n");
        return 1;
    }
    ...
    double* yRef = (double*)calloc(rows, sizeof(double));  // CPU reference result 
    ...
    naiveCsrSpmv<<<gridSize, blockSize>>>(rowPtrGpu, colIdxGpu, valGpu, xGpu, yGpu, rows);
