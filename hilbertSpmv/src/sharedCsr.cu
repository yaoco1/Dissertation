/**
 * @file sharedCsr.cu
 * @brief This file contains the Shared memory CSR SpMV kernel with diagonal x-vector prefetch.
 * It implements a GPU SpMV kernel that prefetches a contiguous window of the input vector x
 * into shared memory before processing each block's rows. For the x[] accesses that fall within
 * the cache window, they are retrieved from the shared memory with low latency. However, 
 * for accesses outside the window, they fall back to the global memory.
 * 
 * @author  Congcong Yao
 * @version 1.0
 * @date    07/08/2026
 ...
 *
 */
__global__ void sharedMemCsrSpmv(const int* __restrict__ rowPtr, const int* __restrict__ colIdx,
    const float* __restrict__ val, const float* __restrict__ x, float* y, int rows, int cols, int sharedVecSize)
{
    extern __shared__ float xShared[];
    int warpIdx = (blockIdx.x * blockDim.x + threadIdx.x) / 32;
    int laneIdx = threadIdx.x % 32;
    int warpsPerBlk = blockDim.x / 32;
    int blockFirstRow = blockIdx.x * warpsPerBlk;
    ...
    for (int i = threadIdx.x; i < sharedVecSize; i += blockDim.x) {
        int global_col = colStart + i;
        xShared[i] = (global_col < cols) ? x[global_col] : 0.0f;
    }
    __syncthreads();
    ...
    int row_start = rowPtr[warpIdx];
    int row_end = rowPtr[warpIdx + 1];
 
    float sum = 0.0f;
    for (int j = row_start + laneIdx; j < row_end; j += 32) {
        int col = colIdx[j];
        float xval;
        if (col >= colStart && col < colEnd)
            xval = xShared[col - colStart];
        else
            xval = x[col];
        sum += val[j] * xval;
    }
...
{
    if (argc < 2) {
        printf("Usage: ./shared_csr <matrix.mtx> [sharedVecSize]\n");
        printf("  sharedVecSize: number of x elements to cache (default: 2048)\n");
        return 1;
    }
 
    int sharedVecSize = (argc >= 3) ? atoi(argv[2]) : 2048;
 
    // Check if the usage of shared memory exceeds the hardware limit
    int maxSharedMem = 0;
    cudaDeviceGetAttribute(&maxSharedMem, cudaDevAttrMaxSharedMemoryPerBlock, 0);
    if ((int)(sharedVecSize * sizeof(float)) > maxSharedMem) {
        printf("Error: sharedVecSize=%d requires %d bytes, but device max is %d bytes\n",
               sharedVecSize, (int)(sharedVecSize * sizeof(float)), maxSharedMem);
        return 1;
    }
    ...
    printf("Shared memory window: %d elements (%.1f KB)\n", sharedVecSize, sharedVecSize * 4.0f / 1024.0f);
    ...
    int gridSize = (rows + warpsPerBlock - 1) / warpsPerBlock;
    int sharedMemBytes= sharedVecSize * sizeof(float);
 
    // Warmup
    cudaMemset(yGpu, 0, rows * sizeof(float));
    sharedMemCsrSpmv<<<gridSize, blockSize, sharedMemBytes>>>(rowPtrGpu, colIdxGpu, valGpu, xGpu, yGpu, rows, cols, sharedVecSize);
    ...
    printf("\n[Shared Memory CSR Kernel] block_size=%d, sharedVecSize=%d:\n", blockSize, sharedVecSize);
    printf("  Time:      %.4f ms\n", avgMs);
